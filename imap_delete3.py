...
- No dependency on 'imap4-utf-7' codec (works fine for Gmail's default ASCII folder names).
- Proper quoting/escaping of mailbox names.
...
def imap_quote_mailbox(name: bytes | str) -> str:
    ...
    - If 'name' is bytes (as returned by LIST), treat it as already IMAP-safe bytes.
    - If it's str, assume ASCII/UTF-8 and best-effort encode to bytes.
    We escape backslashes and double quotes, then wrap in quotes.
    """
    if isinstance(name, (bytes, bytearray)):
        b = bytes(name)
    else:
        # Gmail's default folders are ASCII; for any non-ASCII, we best-effort encode.
        b = name.encode('utf-8', errors='backslashreplace')
    b = b.replace(b'\\', b'\\\\').replace(b'"', b'\\"')
    # We must pass a str to imaplib; decode as latin-1 to preserve byte values 1:1.
    return '"' + b.decode('latin-1') + '"'

def safe_display_name(name: bytes | str) -> str:
    if isinstance(name, (bytes, bytearray)):
        # Best-effort display; Gmail folders are ASCII so this will be fine.
